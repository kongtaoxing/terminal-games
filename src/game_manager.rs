...
impl GameManager {
    ...
    pub fn handle_input(&mut self, key: KeyCode) {
        match self.state {
            ...
            GameState::GoldMiner => self.goldminer.handle_input(key),
            GameState::Tetris => self.tetris.handle_input(key),
        }
    }
    ...
    fn render_main_menu<B: Backend>(&self, f: &mut Frame<B>, area: Rect) {
        let menu_text = vec![
            Spans::from(vec![
                Span::styled("Terminal Game Collection", 
            ...
            Spans::from(""),
            Spans::from(vec![
                Span::styled(" 1. Gold Miner", 
                    Style::default().fg(if self.selected_game == 0 { Color::Green } else { Color::White }))
            ]),
            Spans::from(vec![
                Span::styled(" 2. Tetris", 
                    Style::default().fg(if self.selected_game == 1 { Color::Green } else { Color::White }))
            ]),
            Spans::from(""),
            Spans::from(" Controls:"),
            ...
            Spans::from(""),
            Spans::from(vec![
                Span::styled("Use arrows to select and press Enter!", 
                    Style::default().fg(Color::Yellow))
            ]),
        ];
